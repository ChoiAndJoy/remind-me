...
/**
  * This provides an event sourced behavior. It has a state, [[RemindmeState]], which
  ...
  */
object RemindmeBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[RemindmeCommand]): Behavior[RemindmeCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, RemindmeEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[RemindmeCommand, RemindmeEvent, RemindmeState](
        persistenceId = persistenceId,
        emptyState = RemindmeState.initial,
...
/**
  ...
  */
case class RemindmeState(message: String, timestamp: String) {
  def applyCommand(cmd: RemindmeCommand): ReplyEffect[RemindmeEvent, RemindmeState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: RemindmeEvent): RemindmeState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[RemindmeEvent, RemindmeState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[RemindmeEvent, RemindmeState] =
    Effect
...
object RemindmeState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: RemindmeState = RemindmeState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[RemindmeCommand]("RemindmeAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[RemindmeState] = Json.format
}

/**
  * This interface defines all the events that the RemindmeAggregate supports.
  */
sealed trait RemindmeEvent extends AggregateEvent[RemindmeEvent] {
  def aggregateTag: AggregateEventTag[RemindmeEvent] = RemindmeEvent.Tag
}

object RemindmeEvent {
  val Tag: AggregateEventTag[RemindmeEvent] = AggregateEventTag[RemindmeEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends RemindmeEvent
...
/**
  ...
  */
trait RemindmeCommandSerializable

/**
  * This interface defines all the commands that the RemindmeAggregate supports.
  */
sealed trait RemindmeCommand
    extends RemindmeCommandSerializable
...
/**
  ...
  * application loader.
  */
object RemindmeSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[RemindmeState],
